...
class Backup(
    ...
    suspend fun create(dst: OutputStream): CreateResult =
        withContext(ioDispatcher) {
            ...
            try {
                databaseFile.inputStream().use { src -> dst.use { d -> src.copyTo(d) } }
    ...
    suspend fun restore(src: InputStream): ImportError? =
        withContext(ioDispatcher) {
            val journalFile = databaseFile.resolveSibling("$databaseFile-journal")
            if (journalFile.length() > 0) {
            ...
            try {
                ...
                tmpFile.outputStream().use { dst -> src.use { s -> s.copyTo(dst) } }
